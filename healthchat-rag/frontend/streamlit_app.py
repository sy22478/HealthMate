...
def send_message(message: str):
    ...
    response = requests.post(
        f"{os.environ.get('HEALTHMATE_API_URL', 'http://localhost:8003')}/chat/message",
...
def send_feedback(conversation_id: int, feedback: str):
    ...
    response = requests.post(
        f"{os.environ.get('HEALTHMATE_API_URL', 'http://localhost:8003')}/chat/feedback",
        json=data,
        headers=headers,
        timeout=10
    )
...
def fetch_history():
    ...
    response = requests.get(
        f"{os.environ.get('HEALTHMATE_API_URL', 'http://localhost:8003')}/chat/history",
        headers=headers,
        timeout=10
    )
    if response.status_code == 200:
